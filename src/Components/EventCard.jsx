...
export default function DarkEventCard({ event, isAdmin = false, onEdit, onDelete, onAddToCart }) {
  return (
    ...
    <div className="group relative bg-gray-900 rounded-xl overflow-hidden border border-gray-800 hover:border-purple-500/50 transition-all duration-300 shadow-lg h-full max-w-sm mx-auto">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 pt-8 bg-linear-to-t from-gray-900 via-gray-900/95 to-transparent">
        ...
        {/* Description - Reduced text-sm to text-xs and margins */}
        <p className="text-gray-400 text-xs line-clamp-2 mb-3 leading-relaxed">
            {event.description}
        </p>

        {/* Footer Section - Compacted spacing */}
        <div className="flex items-center justify-between border-t border-gray-700 pt-3">
          ...
          {!isAdmin ? (
            <button 
                ...
                className="bg-white text-gray-900 hover:bg-purple-500 hover:text-white px-3 py-1.5 rounded-lg font-bold text-xs transition-all shadow-md flex items-center gap-1"
            >
                <FaShoppingCart size={10} /> Add
            </button>
          ) : (
            <div className="flex gap-1.5">
               <button onClick={() => onEdit && onEdit(event)} className="bg-gray-800 p-1.5 rounded-md text-gray-400 hover:text-blue-400 hover:bg-gray-700 transition-colors">
                    <FaEdit size={12} />
               </button>
               <button onClick={() => onDelete && onDelete(event.id)} className="bg-gray-800 p-1.5 rounded-md text-gray-400 hover:text-red-400 hover:bg-gray-700 transition-colors">
                    <FaTrash size={12} />
               </button>
            </div>
          )}
